...
#include "pair_sph_taitwater_andrade.h"
#include <cmath>
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairSPHTaitwaterAndrade::PairSPHTaitwaterAndrade(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  single_enable = 0;
  first = 1;
...
PairSPHTaitwaterAndrade::~PairSPHTaitwaterAndrade() {
  if (allocated) {
...
void PairSPHTaitwaterAndrade::compute(int eflag, int vflag) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (delVdotDelR < 0.) {
          ...
          fvisc = -ijvisc * (soundspeed[itype] + soundspeed[jtype]) * mu / (rho[i] + rho[j]);
          // printf("rhoi: %f, ci: %f, wf: %f\n", rho[i], soundspeed[itype], wf);
          // printf("rhoj: %f, cj: %f, h: %f\n", rho[j], soundspeed[jtype], h);
          // printf("ivisc: %f, jvisc: %f, ijvisc: %f\n", ivisc, jvisc, ijvisc);
        } else {
          fvisc = 0.;
        }
...
void PairSPHTaitwaterAndrade::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHTaitwaterAndrade::settings(int narg, char **/*arg*/) {
  if (narg != 0)
    error->all(FLERR,
...
void PairSPHTaitwaterAndrade::coeff(int narg, char **arg) {
  if (narg != 7)
    error->all(FLERR,
...
double PairSPHTaitwaterAndrade::init_one(int i, int j) {
