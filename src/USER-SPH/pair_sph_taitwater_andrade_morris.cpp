...
#include "pair_sph_taitwater_andrade_morris.h"
#include <cmath>
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairSPHTaitwaterAndradeMorris::PairSPHTaitwaterAndradeMorris(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  first = 1;
  single_enable = 0;
...
PairSPHTaitwaterAndradeMorris::~PairSPHTaitwaterAndradeMorris() {
  if (allocated) {
...
void PairSPHTaitwaterAndradeMorris::compute(int eflag, int vflag) {
...
void PairSPHTaitwaterAndradeMorris::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHTaitwaterAndradeMorris::settings(int narg, char **/*arg*/) {
  if (narg != 0)
    error->all(FLERR,
        "Illegal number of arguments for pair_style sph/taitwater/andrade/morris");
}
...
double PairSPHTaitwaterAndradeMorris::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/taitwater/andrade/morris coeffs are not set");
  }
